...
Optomotrist uses Faust for DSP and generates JUCE bridge headers for parameter/state integration.
...
```bash
python3 scripts/codegen.py dsp/optomotrist.dsp --output src/dsp/generated
```

## Build Integration

When `OPTOMOTRIST_ENABLE_CODEGEN=ON`, CMake re-runs codegen when either:

- `dsp/optomotrist.dsp` changes
...
```bash
faust -lang cpp -cn OptomotristDSP -scn "" -vec -vs 32 -lv 1 -ftz 0 -mcd 0 -single -uim
```
