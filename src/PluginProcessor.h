...
class OptomotristAudioProcessor : public juce::AudioProcessor {
public:
  enum class ABSlot { A, B };

  OptomotristAudioProcessor();
  ~OptomotristAudioProcessor() override;
...
juce::ValueTree defaultPresetState;
  ...
  JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(OptomotristAudioProcessor)
};
